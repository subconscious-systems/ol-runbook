...
Creates an EC2 instance, then prepares the host over SSH.
...
```bash
cd gpu-deployment/aws/qwen36-27b-h100-80gb-2gpu
./deploy.sh --help
```

Configuration to review: `AWS_REGION`, `AWS_INSTANCE_TYPE`, `AMI_ID`, `KEY_NAME`, `SUBNET_ID`, `SG_ID`
...
- [Provider implementation](../profiles/_providers/aws.sh) supplies
  the cloud-specific behavior and environment-variable help.
- [Shared host setup](../profiles/_deploy.sh) performs the common steps.

The earlier `profiles/<profile>/aws/deploy.sh` paths remain available.
