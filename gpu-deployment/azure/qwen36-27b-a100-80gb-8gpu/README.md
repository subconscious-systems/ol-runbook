# qwen36-27b-a100-80gb-8gpu on Azure
...
| --- | --- |
| Model | `qwen3.6-27b` |
| Host GPUs | 8 × a100-80gb |
| Workers / GPUs per worker | 1 / 8 |
| Tensor parallelism per worker | 8 |
| CPU / RAM requested per worker | 16 / 128Gi |
| CPU / RAM limit per worker | 64 / 320Gi |
| Image | `registry.distr.sh/subconscious/timrun:sm_89-v2` |
| Model path | `/models/hf/Qwen3.6-27B-FP8` |
| Worker NodePorts | 30001 |
| Helper instance default | `Standard_ND96amsr_A100_v4` |
...
```bash
cd gpu-deployment/azure/qwen36-27b-a100-80gb-8gpu
cp .env.example .env
