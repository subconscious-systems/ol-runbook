# qwen3-8b-l4-1gpu on GCP

The helper has a VM provisioning path for this topology. Verify account access, image, region, quota, networking, and host RAM before running it.
...
| Model | `qwen3-8b` |
| Host GPUs | 1 × l4 |
| Workers / GPUs per worker | 1 / 1 |
| Tensor parallelism per worker | 1 |
| CPU / RAM requested per worker | 2 / 16Gi |
| CPU / RAM limit per worker | 8 / 32Gi |
| Image | `registry.distr.sh/subconscious/timrun:sm_89-v2` |
| Model path | `/models/hf/Qwen3-8B-FP8` |
| Worker NodePorts | 30003 |
| Helper instance default | `g2-standard-24` |
...
```bash
cd gpu-deployment/gcp/qwen3-8b-l4-1gpu
cp .env.example .env
...
worker port(s) (30003) before registering the endpoint with the gateway.
