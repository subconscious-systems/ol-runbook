# glm-5.2-b200-8gpu on GCP

Create the B200 host with the required GCP capacity/provisioning mode, then use `--instance-ip`; the basic VM helper does not implement that full flow.
...
| CPU / RAM requested per worker | 64 / 1363149Mi |
| CPU / RAM limit per worker | 64 / 1363149Mi |
| Image | `registry.distr.sh/subconscious/timrun:sm_100-v1` |
| Model path | `/models/hf/GLM-5.2-FP8` |
| Worker NodePorts | 30001 |
| Helper instance default | `a4-highgpu-8g` |
...
```bash
cd gpu-deployment/gcp/glm-5.2-b200-8gpu
cp .env.example .env
