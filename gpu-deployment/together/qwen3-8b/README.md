# qwen3-8b on Together

Create a matching host through the provider, then use `--instance-ip`. Automatic provisioning is not implemented.
...
| Worker NodePorts | 30003, 30004, 30005, 30006 |
...
```bash
cd gpu-deployment/together/qwen3-8b
cp .env.example .env
...
worker port(s) (30003, 30004, 30005, 30006) before registering the endpoint with the gateway.
