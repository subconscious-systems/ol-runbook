...
install_step_3() {
  ...
  if [[ ! -f "${BOOTSTRAP_DIR}/terraform.tfvars" ]]; then
    cp "${BOOTSTRAP_DIR}/terraform.tfvars.example" \
      "${BOOTSTRAP_DIR}/terraform.tfvars"
    chmod 0600 "${BOOTSTRAP_DIR}/terraform.tfvars"
    printf '[install] created %s from the checked-in example\n' \
      "${BOOTSTRAP_DIR}/terraform.tfvars"
  fi
  cat <<'EOF'

The editor will open terraform.tfvars. Replace every example using this map:

  organization_id OR folder_id (set exactly one)
    Organization: gcloud organizations list
    Folder:       gcloud resource-manager folders list --organization=ORG_ID
    Copy the numeric ID, not the display name.

  billing_account_id
    Run: gcloud billing accounts list --filter=open=true
    Copy the ID shaped 000000-000000-000000. The identity must be allowed to
    attach it to a project.

  quota_project_id
    Use the existing quota project selected in step 2, or an empty string if
    ADC is already configured by policy. Do not put the new project ID here.

  project_id / project_name
    project_id is a new globally unique 6-30 character lowercase GCP ID.
    project_name is its 4-30 character human display name.

  dns_project_id
    Existing project that owns the public zone. Find candidates with
      gcloud projects list
    Then list exact managed-zone resource names with
      gcloud dns managed-zones list --project=DNS_PROJECT_ID

  monthly_budget_amount_usd
    Customer-approved whole-dollar alert threshold; this is not a hard cap.

  region / bootstrap_zone
    Keep us-east1 and an approved us-east1-* zone. The platform uses
    us-east1-b and us-east1-c for N4A nodes.

  bootstrap_subnet_cidr
    Reserved canonical private /24 for the bootstrap VM, for example
    10.40.0.0/24. It must not overlap the platform /16 chosen in step 4.

  operator_principals
    At least one `user:name@example.com` or `group:team@example.com` allowed to
    use IAP and OS Login. Prefer a customer-managed Google Group for day 2.

  project_deletion_policy / protect_bootstrap_vms
    Keep PREVENT and true for production.

terraform.tfvars is ignored and set to mode 0600. It contains identifiers and
configuration only—never paste passwords, PATs, API keys, ADC, or JSON keys.
EOF
  install_wait_for_word 'Have the source values above ready before editing.' ready
