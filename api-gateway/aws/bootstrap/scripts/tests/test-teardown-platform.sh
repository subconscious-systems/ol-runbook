#!/usr/bin/env bash
# Unit tests for bootstrap teardown-platform.sh CLI contract (no AWS/SSM).
set -euo pipefail
...
assert_rc "one name after --yes" 2 teardown_platform_parse_args --yes acme-api-gateway-infra
...
assert_rc "rejects empty-looking bad label" 2 teardown_platform_parse_args \
  --yes -bad acme-api-gateway

echo "== SSM timeout =="
assert_eq "destroy timeout" "$(teardown_platform_ssm_timeout)" "7200"

echo "== remote env wiring (script text) =="
if grep -q -- '--entrypoint /app/scripts/teardown-platform.sh' \
    "${SCRIPTS_DIR}/teardown-platform.sh" \
  && grep -q 'TEARDOWN_CONFIRM=${INFRA_Q}' "${SCRIPTS_DIR}/teardown-platform.sh" \
  && grep -q 'GATEWAY_NAMESPACE=${GATEWAY_Q}' "${SCRIPTS_DIR}/teardown-platform.sh" \
  && grep -q 'docker inspect' "${SCRIPTS_DIR}/teardown-platform.sh" \
  && grep -q 'describe-cluster' "${SCRIPTS_DIR}/teardown-platform.sh" \
  && grep -q 'bootstrap_ssm_run' "${SCRIPTS_DIR}/teardown-platform.sh"; then
  ok "wrapper copies runner env, overrides entrypoint, sets confirm + namespace"
else
