...
#
# The K8s agent runs as pods in the cluster (namespace = DEPLOY_NAME). This host
# is only a kubectl client — it does not host the agent.
#
...
Paste the full Hub Kubernetes-agent connect command (single-quoted).
Namespace must equal DEPLOY_NAME / EKS cluster name (name_prefix).
EOF
}
...
echo "[connect-k8s-agent] deploying Distr K8s agent into EKS namespace ${DEPLOY_NAME}…"
