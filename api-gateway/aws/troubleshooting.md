...
`scripts/sql/usage-lag.sql` compares `max(gateway_usage_events.received_at)` to the latest delivered usage webhook and delivery status counts (see the file header). All `delivered` / HTTP 200 plus an old delivered tip is **delivery-worker lag**, not missing POSTs. After a gateway upgrade, lag can stay high until the one-shot usage catch-up sets `gateway_webhook_usage_catchup.done_at` and the delivery worker POSTs. Schema stays additive; rolling the gateway version back still delivers usage webhooks. Rotate the RDS password if a connection URL or `SecretString` was printed.
