...
# Source before launching Pi so the compaction extension can reach the gateway.
...
case "$COMMAND" in
  install)
    ...
    echo "Source the env file before launching Pi so the extension can reach the gateway:"
