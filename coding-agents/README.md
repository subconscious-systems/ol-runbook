...
./cursor/install.sh
./claude-code/run.sh  (install.sh for persistent config)
./codex/run.sh (install.sh for persistent config)
./opencode/run.sh (install.sh for persistent config)
./pi/install.sh
...
Prefer editing `.env` over passing credentials on the command line.
`install.sh` scripts also accept `--gateway-url` / `--api-key` flags which
override the env file values when you need a one-off.
